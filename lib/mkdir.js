var fs = require('fs')
  ...
  , EMAIL_JADE_BASE_FILE = fs.readFileSync(path.join(__dirname, '../', 'files', 'email.jade'));
;
...
var init = function() {
  ...
  try { fs.writeFileSync(EMAIL_JADE, EMAIL_JADE_BASE_FILE, { flag: 'wx' }); } catch(err) { console.error(err) }
}
